...
course_start_date = st.date_input("Course Start Date", min_value=datetime.today())
...
course_started = st.checkbox("Has the course already started?")
...
if st.button("Calculate Payment Plan"):
    if total_cost > 0:
        payment_plan = calculate_payment_plan(course_start_date.strftime("%Y-%m-%d"), course_end_date.strftime("%Y-%m-%d"), total_cost, num_payments, course_started)
